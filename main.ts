...
async function withRetry<T>(name: string, fn: () => Promise<T>, maxAttempts: number): Promise<T> {
	let lastError: Error;
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		try {
			...
			if (attempt < maxAttempts) {
				...
				core.warning(`${name} failed (attempt ${attempt}/${maxAttempts}), retrying in ${delay}s: ${e instanceof Error ? e.message : String(e)}`);
...
async function run(): Promise<void> {
	try {
		...
		const maxAttempts = Math.max(1, parseInt(core.getInput("max-attempts") || "1", 10) || 1);
		...
		if (resolvedOrNull == null) {
			await core.group("Prepare access to Namespace", async () => {
				await withRetry("installNsc", () => installNsc(), maxAttempts);
			});
		} else {
		...
		await core.group("Log into Namespace workspace", async () => {
			await withRetry("ensureNscloudToken", () => ensureNscloudToken(), maxAttempts);
		});
		...
		if (
			...
			registry = await core.group("Log into Namespace workspace container registry", async () => {
				await withRetry("ensureNscloudToken", () => ensureNscloudToken(), maxAttempts);
				return await withRetry("dockerLogin", () => dockerLogin(), maxAttempts);
			});
		}
